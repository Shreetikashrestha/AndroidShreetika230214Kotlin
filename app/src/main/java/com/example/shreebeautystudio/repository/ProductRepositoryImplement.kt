...
class ProductRepositoryImplement :ProductRepository{

    val database:FirebaseDatabase=FirebaseDatabase.getInstance()

    val ref:DatabaseReference=database.reference.child("products")
    //chaina vane product table banainc=xa cha vane tei use garxa




    override fun addProduct(productModel: ProductModel, callback: (Boolean, String) -> Unit) {
      var id=ref.push().key.toString()//id auto generate garne code
        ...
        ref.addValueEventListener(object:ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (snapshot.exists()) {
                    ...
                    callback(products, true, "products added successfully")
                }
            }
